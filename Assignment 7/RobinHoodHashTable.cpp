...
bool RobinHoodHashTable::insert(const string& elem) {
    if(numElems == numSlots || contains(elem)) return false;
    int hashcode = hashFn(elem);
    string curr = elem;
    for (int i = hashcode,count =0; count < numSlots; i = (i+1)%numSlots,count++) {
        if(elems[i].distance == EMPTY_SLOT){
            elems[i].value = curr;
            elems[i].distance = count;
            numElems++;
            return true;
        }
        else if(count > elems[i].distance){
            swap(elems[i].distance,count);
            swap(elems[i].value,curr);
        }
        else{
            continue;
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for (int i = hashcode,count =0; count < numSlots; i = (i+1)%numSlots,count++) {
        ...
        else if(count > elems[i].distance || elems[i].distance == EMPTY_SLOT){
            return false;
        }
        else{
            continue;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for (int i = hashcode,count =0; count < numSlots; i = (i+1)%numSlots,count++) {
        if(elems[i].value == elem && elems[i].distance != EMPTY_SLOT){
            numElems --;
            elems[i].distance = EMPTY_SLOT;
            while(elems[(i+1)%numSlots].distance > 0){
                swap(elems[i],elems[(i+1)%numSlots]);
                elems[i].distance --;
                i = (i+1)%numSlots;
            }
            return true;

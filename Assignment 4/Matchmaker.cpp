...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    if(possibleLinks.isEmpty()){
        ...
        string now = possibleLinks.firstKey();
        for(auto link:possibleLinks[now]){
            if(possibleLinks.containsKey(link)){   //是否还存在该结点
                //choose,从map中移除这对关系，探索是否有最佳匹配,但不要直接对possibleLinks修改
                auto remain = possibleLinks;
                remain.remove(now);
                remain.remove(link);
                //explore
                if(hasPerfectMatching(remain,matching)){
                    matching.add({now,link});
                    return true;
                }
                //unchoose，无需任何动作
        ...
        //迭代器方式循环不能改变collection内容
        //loop end without true，return false
        return false;
    }
}
//version 2 reference：https://github.com/qwq-fengyue/cs106b-2022-winter
//final test cost:
//  version 2
//      time = 927ms
//  version 1
//      time = 6498ms
void maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks,const Set<string>& remainPerson,Set<Pair>& ans,const Set<Pair>&matching,int& maxweight,int totalweight){
    //base case
    if(remainPerson.size()<2){
    ...
    string p1 = remainPerson.first();
    //choose
    //explore neighbor
    for (string p2 : possibleLinks[p1]) {
        if(remainPerson.contains(p2)){
            Pair newPair = {p1,p2};
            maximumWeightMatchingHelper(possibleLinks,remainPerson -p1 -p2,
                                        ans,matching + newPair,
                                        maxweight,totalweight+possibleLinks[p1][p2]);
        }
    }
    //unchoose
    maximumWeightMatchingHelper(possibleLinks,remainPerson - p1,
                                ans,matching,
                                maxweight,totalweight);
    return ;
}
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    int maxWeight = 0;
    Set<string>remainPerson;
    for (string p : possibleLinks) {
        remainPerson.add(p);
    }
    Set<Pair>ans;
    maximumWeightMatchingHelper(possibleLinks,remainPerson,ans,{},maxWeight,0);
    return ans;
}

/* version1
Set<Pair> maximumWeightMatchingHelper(const Map<string, Map<string, int>> &possibleLinks,const Set<Pair>&matching,int& totalWeight){
    //chosen为目前分组
    //base case: empty
    if(possibleLinks.isEmpty()){
        return matching;
    }
    string chosen = possibleLinks.firstKey();
    Map<Set<Pair>,int>pairStore;
    //unchoose : 不选择该节点
    auto remainLinks = possibleLinks;
    remainLinks.remove(chosen);
    for (string s : possibleLinks) {   //注意循环不能更改迭代器的容器，所以只能用possibLinks
        if(possibleLinks[s].containsKey(chosen)){  //用remainLinks[s]会增添元素
            remainLinks[s].remove(chosen);
        }
    }
    //explore
    int unchooseWeight = totalWeight;
    Set<Pair>unchoose= maximumWeightMatchingHelper(remainLinks,matching,unchooseWeight);
    pairStore[unchoose] = unchooseWeight;
    //choose
    for (string s : possibleLinks[chosen]) {
        int weight = remainLinks[chosen][s];
        if(weight < 0) continue;
        remainLinks = possibleLinks;
        remainLinks.remove(chosen);
        remainLinks.remove(s);
        for (string t : possibleLinks) {
            if(possibleLinks[t].containsKey(chosen)){
                remainLinks[t].remove(chosen);
            }
            if(possibleLinks[t].containsKey(s)){
                remainLinks[t].remove(s);
            }
        }
        //explore
        int chooseWeight = totalWeight + possibleLinks[chosen][s];
        Pair newpair = {chosen,s};
        Set<Pair>choose= maximumWeightMatchingHelper(remainLinks,matching + newpair,chooseWeight) + matching;
        pairStore[choose] = chooseWeight;
    }

    //return max
    int tmp = totalWeight;
    Set<Pair>maxPairs = matching;
    for (Set<Pair> s : pairStore) {
        if(pairStore[s] > tmp){
            tmp = pairStore[s];
            maxPairs = s;
        }
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    int totalWeight = 0;
    return maximumWeightMatchingHelper(possibleLinks,{},totalWeight);
}
*/

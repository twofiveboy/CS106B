...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string> >& roadNetwork, int numCities, Set<string>& supplyLocations,Set<string>& uncoveredCities){
    //base case
    //all covered
    if(uncoveredCities.isEmpty()) return true;
    // num=0 but not all covered
    else if(numCities == 0 && !uncoveredCities.isEmpty())return false;
    //if(roadNetwork.isEmpty()) return true;

    string chosen = uncoveredCities.first();
    //choose
    Set<string>newUncovered = uncoveredCities - chosen -roadNetwork[chosen];
    //explore
    supplyLocations.add(chosen);
    if(canBeMadeDisasterReadyHelper(roadNetwork,numCities - 1,supplyLocations,newUncovered)){
        uncoveredCities = newUncovered;
        return true;
    }
    else{
        supplyLocations.remove(chosen);
    }
    //unchoose 即explore neighbor，neighbor false一定false
    for (string neighbor : roadNetwork[chosen]) {
        newUncovered = uncoveredCities - neighbor - roadNetwork[neighbor];
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities < 0) error("numCities should not be neagtive");
    if(roadNetwork.isEmpty()) return true;
    Set<string>uncoveredCities;
    for (string city : roadNetwork) {
        uncoveredCities.add(city);
    }
    return canBeMadeDisasterReadyHelper(roadNetwork,numCities,supplyLocations,uncoveredCities);
}

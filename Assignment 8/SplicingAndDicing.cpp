...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    Nucleotide* ans = nullptr;
    if(target == nullptr)
        return dna;
    while(dna != nullptr){
        if(dna->value == target->value){
            bool judge = true;
            Nucleotide* tmpDna = dna;
            Nucleotide* tmpTarget = target;
            while(tmpDna != nullptr && tmpTarget != nullptr){
                if(tmpDna->value != tmpTarget->value){
                    judge = false;
                    break;
                }
                tmpDna = tmpDna->next;
                ...
                //长度不匹配
                if(tmpDna == nullptr && tmpTarget != nullptr){
                    judge = false;
                    break;
                }
            }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* findPos = findFirst(dna,target);
    if(findPos == nullptr) return false;
    bool isHead = false;
    if(findPos == dna)
        isHead = true;
    Nucleotide* curr = findPos;

...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(frequencyMap.size() < 2){
        error("less than 2 characters!");
    }
    PriorityQueue<EncodingTreeNode*>pq;
    for (char c : frequencyMap) {
        EncodingTreeNode* Node = new EncodingTreeNode;
        Node->ch = c;
        Node->zero = nullptr;Node->one = nullptr;
        pq.enqueue(Node,frequencyMap[c]);
    }
    while (pq.size() > 1) {
        EncodingTreeNode* lchild,* rchild,* newNode;
        newNode = new EncodingTreeNode;
        int sum = 0;
        lchild = pq.peek();sum += pq.peekPriority();
        pq.dequeue();
        rchild = pq.peek();sum += pq.peekPriority();
        pq.dequeue();
        //enqueue
        newNode->zero = lchild;newNode->one = rchild;
        pq.enqueue(newNode,sum);
    }
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while(bits.size() > 0){
        ...
        while(curr->zero != nullptr || curr->one != nullptr){
            if(bits.peek() == 0){
                curr = curr->zero;
            }
            else{
 ...
 */
void encodeTextHelper(EncodingTreeNode* tree,Map<char,string>&codeMap,string path){
    //reversal encodingtree to get codeMap
    //base case
    if(tree->zero == nullptr && tree->one == nullptr){
        codeMap[tree->ch] = path;
    }
    if(tree->zero != nullptr) encodeTextHelper(tree->zero,codeMap,path+'0');
    if(tree->one != nullptr) encodeTextHelper(tree->one,codeMap,path+'1');
}
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Queue<Bit>q;
    Map<char,string>codeMap;
    encodeTextHelper(tree,codeMap,"");
    for (char c : str) {
        string code = codeMap[c];
        for (char i : codeMap[c]) {
            if(i == '0') q.enqueue(0);
            else q.enqueue(1);
        }
    }
 ...
 * or bits in them, etc.
 */
void decodeTreeHelper(Queue<Bit>& bits, Queue<char>& leaves,EncodingTreeNode* tree){
    //base case
    if(bits.peek() == 0){
        EncodingTreeNode* node = new EncodingTreeNode;
        node->zero = nullptr;node->one = nullptr;
        ...
        bits.dequeue();
        tree->zero = new EncodingTreeNode;
        decodeTreeHelper(bits,leaves,tree->zero);
        bits.dequeue();
        tree->one = new EncodingTreeNode;
        decodeTreeHelper(bits,leaves,tree->one);
    }
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    //base case
    if(bits.peek() == 0){
        tree->zero = nullptr;tree->one = nullptr;
        tree->ch = leaves.dequeue();
    }
    else{
        bits.dequeue();  //dequeue root
        tree->zero = decodeTree(bits,leaves);
        bits.dequeue();  //dequeue lchild or lchild中最右节点
...
STUDENT_TEST("huffmanTreeFor uses cumulative weights (special char ver.).") {
    /*
     *          *
     *       /     \
     *      *       *

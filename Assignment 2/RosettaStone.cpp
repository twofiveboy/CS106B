...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) error("illegal kGramLength");
    else if(kGramLength > str.size())return map;
    for (int i = 0; i + kGramLength <= str.size() ; i++) {
        string tmp = str.substr(i,kGramLength);
        if(map.containsKey(tmp)) map[tmp]++;
        else map[tmp]=1;
    }
    return map;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    double sum = 0,root =0;
    if(input.isEmpty()) error("the map is empty");
    for (string s:input) {
        sum += pow(input[s],2);
    }
    if(sum == 0) error("the containers' values are all zero");
    root = sqrt(sum);
    for (string s:input) {
        output[s] = input[s] / root;
    }
    return output;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    while(!pq.isEmpty()){
        output[pq.peek()] = pq.peekPriority();
        pq.dequeue();
    }
    return output;
}
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    if(lhs.isEmpty() || rhs.isEmpty()) error("map is empty");
    double sum = 0;
    for(string s:lhs){
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    double max = 0;
    string languageName = "";
    for(Corpus now: corpora){
        double tmp = cosineSimilarityOf(textProfile,now.profile);
        if(tmp > max){
            max = tmp;
            languageName = now.name;
        }
    }
    return languageName;
}

...
#include "queue.h"
using namespace std;
bool canVisit(double row,double col,const Grid<double>& terrain,double height){
    if(terrain.inBounds(row,col) && terrain[row][col] <= height ) return true;
    return false;
}
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            const Vector<GridLocation>& sources,
                            double height){
    Grid<bool>isFlooded(terrain.numRows(),terrain.numCols());
    isFlooded.fill(false);
    Queue<GridLocation>q;
    for(GridLocation s:sources){
        if(terrain[s.row][s.col] <= height){
            isFlooded[s.row][s.col] = true;
            q.enqueue(s);
        }
    }
    while (!q.isEmpty()) {
        GridLocation g = q.dequeue();
        if(canVisit(g.row+1,g.col,terrain,height) && isFlooded[g.row+1][g.col] == false){
            isFlooded[g.row+1][g.col] = true;
            q.enqueue(GridLocation(g.row+1,g.col));
        }
        if(canVisit(g.row-1,g.col,terrain,height) && isFlooded[g.row-1][g.col] == false){
            isFlooded[g.row-1][g.col] = true;
            q.enqueue(GridLocation(g.row-1,g.col));
        }
        if(canVisit(g.row,g.col+1,terrain,height) && isFlooded[g.row][g.col+1] == false){
            isFlooded[g.row][g.col+1] = true;
            q.enqueue(GridLocation(g.row,g.col+1));
        }
        if(canVisit(g.row,g.col-1,terrain,height) && isFlooded[g.row][g.col-1] == false){
            isFlooded[g.row][g.col-1] = true;
            q.enqueue(GridLocation(g.row,g.col-1));

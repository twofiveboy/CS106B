...
Vector<DataPoint> merge(Vector<DataPoint>&left,Vector<DataPoint>&right){
    ...
    while(leftIndex<left.size() && rightIndex<right.size()){
        if(left[leftIndex].weight < right[rightIndex].weight)
            v.add(left[leftIndex++]);
        else
            v.add(right[rightIndex++]);
    }
    if(leftIndex < left.size())
        v += left.subList(leftIndex);
    if(rightIndex < right.size())
        v += right.subList(rightIndex);
    return v;
}
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if(size == 0) return {};
    if(size == 1) return sequences[0];
    Vector<DataPoint> v1 = combine(sequences.subList(0,size/2));
    Vector<DataPoint> v2 = combine(sequences.subList(size/2));
    return merge(v1,v2);
//    int i1=0,i2=0;
//    Vector<DataPoint> v;
//    for (int i = 0; i < (v1.size()+v2.size()); i++) {
//        if(i2>=v2.size() || (i1<v1.size() && v1[i1].weight<=v2[i2].weight)){
//            v.add(v1[i1++]);
//        }
//        else{
//            v.add(v2[i2++]);

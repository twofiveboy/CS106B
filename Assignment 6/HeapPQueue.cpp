...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    for (int i = logicalSize; i > 1; i/=2) {
        int parent = i/2;
        if(elems[i].weight < elems[parent].weight){
            swap(elems[i],elems[parent]);
        }
        else{
            break;
        }
...
DataPoint HeapPQueue::peek() const {
    if(isEmpty()) error("empty heap");
    return elems[1];
}

DataPoint HeapPQueue::dequeue() {
    DataPoint returnPoint = peek();
    swap(elems[1],elems[logicalSize]);
    logicalSize --;
    //bubble down
    for (int i = 1; 2*i <= logicalSize;) {
        int lchild = 2*i,rchild = 2*i+1;
        if(lchild > logicalSize && rchild > logicalSize){  // no child
            break;
        }
        else if(rchild <= logicalSize){  //two children
            if(elems[lchild].weight <= elems[rchild].weight && elems[lchild].weight < elems[i].weight){
                swap(elems[i],elems[lchild]);
                i = lchild;
            }
            else if(elems[rchild].weight <= elems[lchild].weight && elems[rchild].weight < elems[i].weight){
                swap(elems[i],elems[rchild]);
                i = rchild;
            }
            else {
                break;
        ...
        else{ //one child(must be left)
            if(elems[lchild].weight < elems[i].weight){
                swap(elems[i],elems[lchild]);
                i = lchild;
            }
            else{

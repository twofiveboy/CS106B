...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if(numSeats < populations.size()) error("not enough seats");
    ...
    for (string s : populations) {
        DataPoint p;
        p.name = s;
        p.weight = -(populations[s]/sqrt(2));
        h.enqueue(p);
        ans[s] = 1;
    }
    while(numSeats--){
        DataPoint now = h.dequeue();
        ans[now.name]++;
        now.weight = -(populations[now.name]/sqrt(ans[now.name]*(ans[now.name]+1)));
        h.enqueue(now);
    }
    return ans;
